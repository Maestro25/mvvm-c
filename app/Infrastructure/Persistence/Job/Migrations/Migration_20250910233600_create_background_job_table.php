...
/**
 * Migration_20250910233600_create_background_job_table
 *
 * Creates 'background_job' table with core async job data.
 * Audit events logged via centralized audit_log table through triggers.
 */
class Migration_20250910233600_create_background_job_table implements MigrationInterface
{
    ...
    {
        return 'Creates background_job table for async jobs; audit via global audit_log.';
    }
    ...
    public function up(SafeMySQL $db): void
    {
        $db->query(<<<SQL
            CREATE TABLE IF NOT EXISTS background_job (
                ...
                status CHAR(36) NOT NULL COMMENT 'FK to job_status(id)',
                ...
                created_ip VARBINARY(16) DEFAULT NULL COMMENT 'Creator IP address (IPv4/IPv6)',

                updated_at TIMESTAMP(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6) COMMENT 'Record last update timestamp',
                updated_by BINARY(16) DEFAULT NULL COMMENT 'UUID of last updater',
                updated_ip VARBINARY(16) DEFAULT NULL COMMENT 'Updater IP address (IPv4/IPv6)',
                ...
                deleted_ip VARBINARY(16) DEFAULT NULL COMMENT 'Deleter IP address',

                version INT UNSIGNED NOT NULL DEFAULT 1 COMMENT 'Optimistic concurrency/version control',

                CONSTRAINT fk_background_job_status FOREIGN KEY (status) REFERENCES job_status(id) ON DELETE RESTRICT ON UPDATE CASCADE,

                INDEX idx_background_job_status (status),
                INDEX idx_background_job_scheduled_at (scheduled_at)
            ) ENGINE=InnoDB CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
            COMMENT='Tracking for asynchronous background jobs';
        SQL);
    }

    public function down(SafeMySQL $db): void
    {
        $db->query("DROP TABLE IF EXISTS background_job;");
    }
}
